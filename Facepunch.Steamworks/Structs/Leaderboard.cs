...
public struct Leaderboard
{
	...
	private static readonly int[] detailsBuffer = new int[64];
	private static readonly int[] noDetails = Array.Empty<int>();
	...
	public async Task<LeaderboardUpdate?> SubmitScoreAsync( int score, int[] details = null )
	{
		...
		var r = await SteamUserStats.Internal.UploadLeaderboardScore( Id, LeaderboardUploadScoreMethod.KeepBest, score,
	...
	{
		var r = await SteamUserStats.Internal.DownloadLeaderboardEntries( Id, LeaderboardDataRequest.GlobalAroundUser,
			start, end );
		if ( !r.HasValue )
		{
			return null;
