...
public class SteamTimeline : SteamClientClass<SteamTimeline>
{
	...
	///     timeline when saving clips. Setting a
	...
	/// <summary>
	///     Use this to mark an event on the Timeline. This event will be instantaneous. (See
	///     <see cref="AddRangeTimelineEvent" /> to add events that happened over time.)
	/// </summary>
	...
	/// <summary>
	///     Use this to mark the start of an event on the Timeline that takes some amount of time to complete. The duration of
	///     the event is determined by a matching call
	///     to <see cref="EndRangeTimelineEvent" />. If the game wants to cancel an event in progress, they can do that with a
	///     call to <see cref="RemoveTimelineEvent" />.
	/// </summary>
	...
	/// <summary>
	///     Use this to determine if video recordings exist for the specified event. This can be useful when the game needs to
	///     decide whether or not to show a control
	...
	/// <summary>
	///     Use this to start a game phase. Game phases allow the user to navigate their background recordings and clips.
	///     Exactly what a game phase means will vary game
	///     to game, but the game phase should be a section of gameplay that is usually between 10 minutes and a few hours in
	///     length, and should be the main way a user
	///     would think to divide up the game. These are presented to the user in a UI that shows the date the game was played,
	///     with one row per game slice. Game phases
	...
	///     <see cref="DoesGamePhaseRecordingExist" /> or
	///     <see cref="OpenOverlayToGamePhase" />. It may also be used to associated multiple phases with each other.
	/// </summary>
	/// <param name="phaseId">
	///     A game-provided persistent ID for a game phase. This could be a the match ID in a multiplayer
	///     game, a chapter name in a single player game, the ID of a character, etc.
	/// </param>
	...
	/// <summary>
	///     Use this to determine if video recordings exist for the specified game phase. This can be useful when the game
	///     needs to decide whether or not to show a control that will call <see cref="OpenOverlayToGamePhase" />.
	/// </summary>
	...
	/// <summary>
	///     Use this to add a game phase tag. Phase tags represent data with a well defined set of options, which could be data
	///     such as match resolution, hero played, game mode, etc. Tags can have an icon
	///     in addition to a text name. Multiple tags within the same group may be added per phase and all will be remembered.
	///     For example, this may be called multiple times for a "Bosses Defeated" group,
	...
	/// <summary>
	///     Use this to add a game phase attribute. Phase attributes represent generic text fields that can be updated
	///     throughout the duration of the phase. They are meant to be used for phase metadata
	///     that is not part of a well defined set of options. For example, a KDA attribute that starts with the value "0/0/0"
	///     and updates as the phase progresses, or something like a played-entered character
	///     name. Attributes can be set as many times as the game likes with SetGamePhaseAttribute, and only the last value
	...
	/// <summary>
	///     Opens the Steam overlay to the section of the timeline represented by the timeline event. This event must be in the
	///     current game session, since <see cref="TimelineEventHandle" /> values are not
