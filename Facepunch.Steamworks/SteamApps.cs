...
public class SteamApps : SteamSharedClass<SteamApps>
{
	...
	/// <summary>
	///     Gets whether or not the user borrowed this game via Family Sharing. If true, call GetAppOwner() to get the lender
	///     SteamID.
	/// </summary>
	...
	/// <summary>
	///     Gets the name of the beta branch that is launched, or <see langword="null" /> if the application is not running on
	///     a beta branch.
	/// </summary>
	...
	/// <summary>
	///     Gets whether or not the app is owned by the current user. The app may not actually be owned by the current user;
	...
	///     <para>
	///         Parameter names starting with the character '<c>@</c>' are reserved for internal use and will always return an
	///         empty string.
	///         Parameter names starting with an underscore '<c>_</c>' are reserved for steam features -- they can be queried
	///         by the game,
