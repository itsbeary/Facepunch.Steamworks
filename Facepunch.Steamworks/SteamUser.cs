...
public class SteamUser : SteamClientClass<SteamUser>
{
	...
	private static readonly byte[] readBuffer = new byte[1024 * 128];
	...
	public static unsafe int DecompressVoice( Stream input, int length, Stream output )
	{
		...
		//
		using ( var s = new MemoryStream( from ) )
		{
			input.CopyTo( s );
		}
	...
	public static unsafe int DecompressVoice( byte[] from, Stream output )
	{
		...
		fixed ( byte* dst = to )
		{
			if ( Internal.DecompressVoice( (IntPtr)frm, (uint)from.Length, (IntPtr)dst, (uint)to.Length, ref szWritten,
				    SampleRate ) != VoiceResult.OK )
			{
				return 0;
			}
	...
	public static int DecompressVoice( IntPtr from, int length, IntPtr to, int bufferSize )
	{
		...
		if ( Internal.DecompressVoice( from, (uint)length, to, (uint)bufferSize, ref szWritten, SampleRate ) !=
		     VoiceResult.OK )
		{
			return 0;
		}
	...
	/// </summary>
	public static async Task<AuthTicket> GetAuthSessionTicketAsync( NetIdentity identity,
		double timeoutSeconds = 10.0f )
	{
	...
	private static AuthTicket GetAuthTicketForWebApi( string identity )
	{
		...
		return new AuthTicket { Handle = ticket };
	}
