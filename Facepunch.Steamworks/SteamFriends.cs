...
public class SteamFriends : SteamClientClass<SteamFriends>
{
	...
	/// <summary>
	///     Invoked when a chat message has been received in a Steam group chat that we are in. Associated Functions:
	...
	private static void OnGameConnectedClanChatMessage( GameConnectedClanChatMsg_t data )
	{
		...
		var len = Internal.GetClanChatMessage( data.SteamIDClanChat, data.MessageID, buffer, Helpers.MemoryBufferSize,
			ref type, ref chatter );
	...
	{
		Internal.ActivateGameOverlayToWebPage( url,
			modal ? ActivateGameOverlayToWebPageMode.Modal : ActivateGameOverlayToWebPageMode.Default );
	}
