...
{
	private static readonly List<SteamClass> openInterfaces = new();
	...
	private static readonly Dictionary<string, string> KeyValue = new();
	...
	/// <summary>
	///     Invoked when authentication status changes, useful for grabbing <see cref="SteamId" /> once availability is
	///     current.
	/// </summary>
	...
	///     If <paramref name="asyncCallbacks" /> is <see langword="false" /> you need to call <see cref="RunCallbacks" />
	...
	public static void Init( AppId appid, SteamServerInit init, bool asyncCallbacks = true )
	{
		...
		var result = SteamInternal.GameServer_Init( ipaddress, init.GamePort, init.QueryPort, secure,
			init.VersionString, interfaceVersions, out var error );
		if ( result != SteamAPIInitResult.OK )
		{
			throw new Exception(
				$"InitGameServer({ipaddress},{init.GamePort},{init.QueryPort},{secure},\"{init.VersionString}\") returned false - error: {error}" );
		}
