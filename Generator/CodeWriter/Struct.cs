...
{
	private readonly Dictionary<string, TypeDef> TypeDefs = new();
	...
	private void Structs()
	{
		foreach ( var c in def.structs )
		{
			...
			//
			WriteLine(
				$"[StructLayout( LayoutKind.Sequential, Pack = Platform.{(c.IsPack4OnWindows ? "StructPackSize" : "StructPlatformPackSize")} )]" );
	...
	private void StructFields( SteamApiDefinition.StructDef.StructFields[] fields )
	{
		foreach ( var m in fields )
		{
			...
			{
				WriteLine(
					$"internal string {CleanMemberName( m.Name )}UTF8() => Steamworks.Utility.Utf8NoBom.GetString( {CleanMemberName( m.Name )}, 0, System.Array.IndexOf<byte>( {CleanMemberName( m.Name )}, 0 ) );" );
			...
			if ( t.StartsWith( "ushort " ) && t.Contains( "[" ) )
			{
				...
				t = "ushort[]";
				WriteLine(
					$"[MarshalAs(UnmanagedType.ByValArray, SizeConst = {num}, ArraySubType = UnmanagedType.U2)]" );
			}
			...
			if ( t.StartsWith( "SteamId" ) && t.Contains( "[" ) )
			{
				...
				t = "ulong[]";
				WriteLine(
					$"[MarshalAs(UnmanagedType.ByValArray, SizeConst = {num}, ArraySubType = UnmanagedType.U8)]" );
			}
			...
			if ( t.StartsWith( "uint32 " ) && t.Contains( "[" ) )
			{
				...
				t = "uint[]";
				WriteLine(
					$"[MarshalAs(UnmanagedType.ByValArray, SizeConst = {num}, ArraySubType = UnmanagedType.U4)]" );
			}
			...
			if ( t.StartsWith( "float " ) && t.Contains( "[" ) )
			{
				...
				t = "float[]";
				WriteLine(
					$"[MarshalAs(UnmanagedType.ByValArray, SizeConst = {num}, ArraySubType = UnmanagedType.R4)]" );
			}
