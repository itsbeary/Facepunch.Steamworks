...
{
	private void StructFunctions()
	{
		foreach ( var c in def.structs.Union( def.callback_structs.Select( x => x as SteamApiDefinition.StructDef ) )
			         .OrderBy( x => x.Name ) )
		{
			...
			StartBlock( $"{Cleanup.Expose( name )} partial struct {name}" );
			{
				foreach ( var func in c.Methods )
				{
					...
					WriteLine(
						$"internal static{_unsafe} extern {returnType.TypeNameFrom} Internal{func.Name}( {firstArg}, {delegateargstr} );"
							.Replace( $"( {firstArg},  )", $"( {firstArg} )" ) );
					WriteLine();
				}
			}
			EndBlock();
