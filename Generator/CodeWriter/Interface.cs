...
public partial class CodeWriter
{
	public void GenerateInterface( SteamApiDefinition.Interface iface, string folder )
	{
		...
		StartBlock( "namespace Steamworks" );
		{
			StartBlock( $"internal unsafe partial class {iface.Name} : SteamInterface" );
			{
				...
				if ( iface.Accessors != null )
				{
					...
					{
						WriteLine(
							$"[DllImport( Platform.LibraryName, EntryPoint = \"{func.Name_Flat}\", CallingConvention = Platform.CC)]" );
		...
		File.WriteAllText( $"{folder}{iface.Name}.cs", sb.ToString() );
	}
	...
	private void WriteFunction( SteamApiDefinition.Interface iface, SteamApiDefinition.Interface.Method func )
	{
		...
		WriteLine(
			$"[DllImport( Platform.LibraryName, EntryPoint = \"{func.FlatName}\", CallingConvention = Platform.CC)]" );
		...
		WriteLine( $"private static extern {returnType.TypeNameFrom} _{func.Name}( IntPtr self, {delegateargstr} );"
			.Replace( "( IntPtr self,  )", "( IntPtr self )" ) );
