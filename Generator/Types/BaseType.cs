...
internal class BaseType
{
	...
	public virtual string Ref =>
		(!TreatAsPointer && !IsVector && NativeType.EndsWith( "*" )) || NativeType.EndsWith( "**" ) ||
		NativeType.Contains( "&" )
			? "ref "
			: "";
	...
	public virtual bool IsVector
	{
		get
		{
			...
			if ( Func == "ReadP2PPacket" )
			{
				return false;
			}

			if ( Func == "SendP2PPacket" )
			{
				return false;
			}

			if ( VarName == "pOutMessageNumber" )
			{
				return false;
			}
			...
			if ( VarName == "pLanePriorities" )
			{
				return true;
			}

			if ( VarName == "pLaneWeights" )
			{
				return true;
			}
			...
			if ( VarName == "pOutResultHandle" )
			{
				return false;
			}

			if ( VarName == "pOutDataType" )
			{
				return false;
			}

			if ( VarName == "psteamIDClans" )
			{
				return true;
			}

			if ( VarName == "pScoreDetails" )
			{
				return true;
			}
			...
			if ( VarName == "pBasePrices" )
			{
				return true;
			}

			if ( VarName == "pCurrentPrices" )
			{
				return true;
			}

			if ( VarName == "pItemDefIDs" )
			{
				return true;
			}

			if ( VarName == "handlesOut" )
			{
				return true;
			}

			if ( VarName == "pDetails" && Func == "GetDownloadedLeaderboardEntry" )
			{
				return true;
			}

			if ( VarName == "pData" && NativeType.EndsWith( "*" ) && Func.StartsWith( "GetGlobalStatHistory" ) )
			{
				return true;
			}
			...
			if ( VarName.StartsWith( "pArray" ) )
			{
				return true;
			}

			if ( VarName.StartsWith( "punArray" ) )
			{
				return true;
			}
			...
			if ( NativeType.EndsWith( "*" ) )
			{
				if ( VarName.StartsWith( "pvec" ) )
				{
					return true;
				}

				if ( VarName.StartsWith( "pub" ) )
				{
					return true;
				}

				if ( VarName.StartsWith( "pOut" ) )
				{
					return true;
				}
			}
		...
		string bufferSizeName = null )
	{
		...
		{
			return new PointerType { NativeType = "void *", VarName = varname };
		}

		if ( type == "SteamAPIWarningMessageHook_t" )
		{
			return new PointerType { NativeType = type, VarName = varname };
		}

		if ( typeNoSpaces == "MatchMakingKeyValuePair**" )
		{
			return new PointerType { NativeType = "MatchMakingKeyValuePair_t", VarName = varname };
		}

		if ( type == "SteamAPICall_t" )
		{
			return new SteamApiCallType { NativeType = type, VarName = varname, CallResult = callresult };
		}

		if ( type == "void" )
		{
			return new VoidType { NativeType = type, VarName = varname };
		}

		if ( typeNoSpaces.StartsWith( "constchar*" ) )
		{
			return new ConstCharType { NativeType = type, VarName = varname };
		}

		if ( type == "char *" )
		{
			return new FetchStringType { NativeType = type, VarName = varname, BufferSizeParamName = bufferSizeName };
		}
		...
		{
			return new IntType { NativeType = type, VarName = varname };
		}

		if ( basicType == "uint" )
		{
			return new UIntType { NativeType = type, VarName = varname };
		}

		if ( basicType == "uint8" )
		{
			return new UInt8Type { NativeType = type, VarName = varname };
		}

		if ( basicType == "uint16" )
		{
			return new UInt16Type { NativeType = type, VarName = varname };
		}
		...
		if ( basicType == "size_t" )
		{
			return new UIntPtrType { NativeType = type, VarName = varname };
		}

		if ( basicType == "uint64" )
		{
			return new ULongType { NativeType = type, VarName = varname };
		}

		if ( basicType == "int64" )
		{
			return new LongType { NativeType = type, VarName = varname };
		}

		if ( basicType == "bool" )
		{
			return new BoolType { NativeType = type, VarName = varname };
		}

		//
		// Enum types are handled in a generic way, but we do need to clean up the name
		//
		if ( CodeWriter.Current.IsEnum( basicType ) )
		{
		...
		//
		if ( CodeWriter.Current.IsStruct( basicType ) )
		{
		...
		//
		if ( CodeWriter.Current.IsTypeDef( basicType ) )
		{
			return new StructType { NativeType = type, VarName = varname, StructName = basicType };
		}
	...
	public virtual string AsNativeArgument()
	{
		return AsArgument();
	}
	...
	public virtual string AsCallArgument()
	{
		return $"{Ref}{VarName}";
	}

	public virtual string Return( string varname )
	{
